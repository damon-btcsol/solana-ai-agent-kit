...
def tokens_for_sol(
    ...
    """
    Calculate the amount of SOL received for selling a given amount of tokens.

    Args:
        sell_token_amount (float): The amount of tokens to sell.
        base_vault_balance (float): The balance of the base vault.
        quote_vault_balance (float): The balance of the quote vault.
        swap_fee (float): The swap fee percentage (default is 0.25%).

    Returns:
        Optional[float]: The amount of SOL received, rounded to 9 decimal places, or None if an error occurs.
    """
    try:
        if sell_token_amount <= 0 or base_vault_balance <= 0 or quote_vault_balance <= 0:
            raise ValueError("All input amounts must be positive numbers.")
        ...
        print(f"Error calculating tokens for SOL: {e}")
        return None